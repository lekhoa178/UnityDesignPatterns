...
public class GoapAgent : MonoBehaviour, Mediator.IVisitable
{
    [Header("Sensors")]
    ...
    void SetupActions()
    {
        actions = new HashSet<AgentAction>
        {
            ...
            new AgentAction.Builder("Move To Eating Position")
            ...
            new AgentAction.Builder("MoveFromDoorDoorToRestArea")
            .WithStrategy(new MoveStrategy(navMeshAgent, () => restingPosition.position))
            .WithCost(2)
    ...
    bool InRangeOf(Vector3 pos, float range) => Vector3.SqrMagnitude(transform.position - pos) < range * range;
    ...
    private void Update()
    {
        ...
        // Update the plan and the current action if there is one
        if (currentAction == null)
        {
            ...
            if (actionPlan != null && actionPlan.Actions.Count > 0)
            {
                ...
                {
                    Debug.Log("Preconditions not met, clearing currnet action and goal");
                    currentAction = null;
                    currentGoal = null;
                }
            }
    ...
    public void Accept(Mediator.IVisitor visitor)
    {
