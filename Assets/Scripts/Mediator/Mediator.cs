...
public abstract class Mediator<T> : MonoBehaviour where T : Component, Mediator.IVisitable
{
    ...
    public void Message(T source, T target, Mediator.IVisitor message)
    {
        entities.FirstOrDefault(entity => entity.Equals(target)).Accept(message);
    }
    public void Broadcast(T source, Mediator.IVisitor message, Func<T, bool> predicate = null)
    {
